    ...
    <div id="projects" className="w-full pb-[100px]">
      <div className="w-3/5 mx-auto text-center space-y-2 mb-5">
        <Fade delay={200} duration={500} fraction={0.5} distance="0px">
          ...
          <p className="text-sm">
            A collection of projects I’ve built to sharpen my web development
            skills. Each one helped me explore new tools, frameworks, and
            challenges.
          </p>
        </Fade>
      </div>
      ...
      <div className=" w-full md:w-10/12 lg:w-10/12 mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-2">
        <Suspense
          fallback={
            <div className="w-full h-[300px] content-center">
              <span className="loading loading-spinner text-error"></span>
            </div>
          }
        >
          {project.map((pro) => (
            <SingleProject key={pro.id} pro={pro}></SingleProject>
          ))}
        </Suspense>
      </div>
    </div>
